...
export const getPricesTool = tool({
  ...
  parameters: z.object({
    action: z
      ...
      .describe("Whether to fetch price for a single coin or multiple coins"),
    coins: z
      .array(z.string())
      .min(1)
      .describe(
  ...
  execute: async ({ action, coins, detailed }) => {
    ...
    const requestedCoins = coins.filter((coin) =>
    ...
    const priceMethodMap = {
      detailed: {
        ...
        multipleCoins: () =>
          prices.getCoinsToPriceInfo({ coins: requestedCoins }),
      },
      basic: {
